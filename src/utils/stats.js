...
export function meaningfulSessions(sessions) {
  return sessions.filter(
    (session) => session.meaningful && Number.isFinite(session.durationMs),
  );
}
...
export function calculateStats(sessions, now = new Date()) {
  ...
  const mostUsedId =
    Object.keys(byMode).sort((a, b) => byMode[b] - byMode[a])[0] || null;
  return {
    ...
    activeDays: new Set(
      meaningful.map((session) => localDateKey(session.endedAt)),
    ).size,
    byMode,
    mostUsedId,
  };
